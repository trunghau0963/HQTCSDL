...
EXEC INSERT_INTO_TOATHUOC 'AA8630F6-698E', N'Amoxicillin-300G', 30, 'A'
EXEC UPDATE_INFO_THUOC 'D1981DCA-61C', 'A104150C-9', 'Amoxicillin-300g', N'vIÊN', 'CC', 200, '2024-12-12', 12200
...
BEGIN TRAN 
    DECLARE @MSG NVARCHAR(64)
    BEGIN TRY
        IF NOT EXISTS(SELECT * FROM DICHVU DV WHERE DV.TENDV COLLATE SQL_Latin1_General_CP1_CI_AI LIKE '%' + @TENDV + '%' AND DAXOA = 0)
        BEGIN
            SELECT *
            FROM DICHVU DV
...
BEGIN TRAN 
    DECLARE @MSG NVARCHAR(64)
    BEGIN TRY
        IF NOT EXISTS(SELECT * FROM THUOC T WHERE T.TENTHUOC COLLATE SQL_Latin1_General_CP1_CI_AI LIKE '%' + @TENTHUOC + '%' AND DAXOA = 0)
        BEGIN
        	    SELECT *
                FROM THUOC T
...
BEGIN TRAN 
    DECLARE @MSG NVARCHAR(64)
    BEGIN TRY
        IF NOT EXISTS(SELECT * FROM BENHNHAN BN WHERE BN.HOTEN COLLATE SQL_Latin1_General_CP1_CI_AI LIKE '%' + @HOTEN + '%')
        BEGIN
            SELECT *
            FROM BENHNHAN BN
...
exec CHANGE_QUANTITY_THUOC 'D1981DCA-61C', 'A104150C-9', 10, '+'
--Lost update 02
--tran01
GO
