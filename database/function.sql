...
LẤY THÔNG TIN LỊCH LÀM VIÊC CÒN TRỐNG THEO NGAY
    GET_LICHLAMVIEC_DETAIL_FREE_BY_DATE @NGAY DATE
...
KHÓA BỆNH NHÂN, NHA SĨ, NHÂN VIÊN
    BLOCK_ACCOUNT @MA CHAR(16)
MỞ KHÓA BỆNH NHÂN, NHA SĨ, NHÂN VIÊN
    UNBLOCK_ACCOUNT @MA CHAR(16)
*/
--FUNCTION
GO
...
go 
CREATE OR ALTER PROC GET_LICHLAMVIEC_DETAIL_FREE_BY_DATE @DATE DATE
AS
BEGIN TRAN
    DECLARE @ErrorMessage NVARCHAR(4000);
    DECLARE @MSG NVARCHAR(64)
    BEGIN TRY
        ...
        FROM LICHLAMVIEC LLV
        JOIN NHASI NS ON NS.MANS = LLV.MANS
        WHERE LLV.NGAYKHAM = @DATE AND NOT EXISTS(
            SELECT * 
            FROM LICHKHAM LK
            WHERE LK.MANS = LLV.MANS AND LK.GIOKHAM = LLV.GIOKHAM
                    AND LK.NGAYKHAM = LLV.NGAYKHAM)
